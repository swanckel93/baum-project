...
from .enums import Currency, QuoteStatus
...
class Quote(BaseModel):
    ...
    currency: Mapped[Currency] = mapped_column(
        Enum(Currency), nullable=False, default=Currency.EUR
    )
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    status: Mapped[QuoteStatus] = mapped_column(
        Enum(QuoteStatus), nullable=False, default=QuoteStatus.PENDING
    )
    margin_percentage: Mapped[float | None] = mapped_column(
    ...
    craftsman_id: Mapped[int] = mapped_column(
        ForeignKey("craftsmen.id"), nullable=False
    )
