...
def read_campaigns(
    ...
    if project_id:
        campaigns = campaign_service.get_by_project(db, project_id=project_id, skip=skip, limit=limit)
    elif status_filter:
        campaigns = campaign_service.get_by_status(db, status=status_filter, skip=skip, limit=limit)
    elif active_only:
