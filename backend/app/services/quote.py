...
class QuoteService(BaseCRUDService[Quote, QuoteCreate, QuoteUpdate]):
    ...
    def get_by_item(self, db: Session, *, item_id: int, skip: int = 0, limit: int = 100) -> List[Quote]:
        """Get quotes by item ID"""
        return db.query(Quote).filter(
            Quote.item_id == item_id
        ).offset(skip).limit(limit).all()
    
    def get_by_craftsman(self, db: Session, *, craftsman_id: int, skip: int = 0, limit: int = 100) -> List[Quote]:
    ...
    def get_pending(self, db: Session, *, skip: int = 0, limit: int = 100) -> List[Quote]:
        """Get pending quotes"""
        return db.query(Quote).filter(
            Quote.status == QuoteStatus.PENDING
        ).offset(skip).limit(limit).all()
    
    def get_approved(self, db: Session, *, skip: int = 0, limit: int = 100) -> List[Quote]:
        """Get approved quotes"""
        return db.query(Quote).filter(
