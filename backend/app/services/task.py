...
class TaskService(BaseCRUDService[Task, TaskCreate, TaskUpdate]):
    ...
    def get_by_priority(self, db: Session, *, priority: TaskPriority, skip: int = 0, limit: int = 100) -> List[Task]:
    ...
    def get_in_progress(self, db: Session, *, skip: int = 0, limit: int = 100) -> List[Task]:
        """Get in-progress tasks"""
        return db.query(Task).filter(
            Task.status == TaskStatus.IN_PROGRESS
        ).offset(skip).limit(limit).all()
    
    def get_unassigned(self, db: Session, *, skip: int = 0, limit: int = 100) -> List[Task]:
